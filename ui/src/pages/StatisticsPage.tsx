...
interface DashboardData {
    ...
    distractions: {
        blockedCount: number;
        topDistractions: { name: string; time: string; count: number }[];
    };
...
const MOCK_DASHBOARD_DATA: DashboardData = {
    ...
    distractions: {
        blockedCount: 18,
        topDistractions: [
            { name: "YouTube", time: "1h 20m", count: 120 },
            { name: "Twitter", time: "45m", count: 85 },
            { name: "Reddit", time: "30m", count: 60 },
            { name: "Other", time: "15m", count: 40 }
        ]
    },
    tasks: [
...
export default function StatisticsPage() {
    ...
    return (
        <div
            className="h-screen w-screen p-2 sm:p-3 md:p-4 lg:p-6 bg-dash-dark bg-cover bg-center bg-no-repeat overflow-hidden flex flex-col font-sans selection:bg-dash-lime/30"
            style={{
                backgroundImage: `linear-gradient(rgba(6,22,21,0.5), rgba(6,22,21,0.5)), var(--synapse-bg-image)`,
                ...
                >
                    ...
                    <div className="flex flex-col gap-2 sm:gap-3 md:gap-4 min-h-0">
                        <div className="flex items-center gap-2 sm:gap-3 md:gap-4 flex-shrink-0">
                            <button
                                onClick={() => navigate('/')}
                                className="p-1.5 sm:p-2 md:p-2.5 bg-dash-dark hover:bg-dash-forest text-dash-lime rounded-full transition-all flex items-center justify-center shadow-lg active:scale-95 group"
                        ...
                        <div className="bg-dash-dark rounded-lg sm:rounded-xl md:rounded-2xl p-3 sm:p-4 md:p-5 flex flex-col justify-between flex-1 min-h-0">
                            <div className="flex flex-col mb-6 sm:mb-8">
                                <h2 className="text-sm sm:text-base md:text-lg lg:text-xl font-semibold text-dash-lime tracking-tight leading-tight">
                                    Weekly Summary
                                </h2>
                                <p className="text-xs sm:text-sm md:text-base text-dash-moss font-medium">Total Week's Time: <span className="text-dash-lime font-bold">{MOCK_DASHBOARD_DATA.weeklySummary.totalTime}</span></p>
                            </div>
                                        ...
                                        <>
                                            ...
                                            <div className="absolute inset-x-0 top-0 bottom-6 pointer-events-none">
                                                {gridValues.map((val, i) => (
                                                    ...
                                                    >
                                                        <span className="text-[7px] sm:text-[8px] text-white/50 font-medium whitespace-nowrap min-w-[28px] -translate-y-1/2">
                                                            {val % 1 === 0 ? `${val}h` : `${val.toFixed(1)}h`}
                                                        </span>
                                                        <div className="flex-1 border-t border-dash-olive/10 sm:border-dash-olive/20 border-dashed"></div>
                                                    </div>
                                                ))}
                                            </div>
                                                ...
                                                {MOCK_DASHBOARD_DATA.weeklySummary.history.map((item, i) => {
                                                    ...
                                                    return (
                                                        <div key={i} className="flex-1 h-full flex flex-col justify-end items-center relative">
                                                            <div
                                                                className={`group w-full max-w-[14px] sm:max-w-[18px] md:max-w-[24px] ${isToday ? 'bg-dash-lime' : 'bg-dash-forest'} rounded-[3px] relative transition-all duration-300`}
                                                            ...
                                                            >
                                                                ...
                                                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20">
                                                                    <div className="bg-dash-lime text-dash-dark text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded shadow-lg whitespace-nowrap">
                                                                        {item.hours % 1 === 0 ? `${item.hours}h` : `${item.hours.toFixed(1)}h`}
                                                                    </div>
                                                                </div>
                                                                <div className="absolute inset-0 bg-gradient-to-t from-dash-dark/20 to-transparent"></div>
                                                            </div>
                                                        </div>
                                                    );
                                                })}
                                                ...
                                                {MOCK_DASHBOARD_DATA.weeklySummary.history.map((item, i) => {
                                                    ...
                                                    return (
                                                        <div key={i} className="flex-1 flex justify-center items-end">
                                                            <span className={`text-[8px] sm:text-[10px] ${isToday ? 'text-white font-bold' : 'text-white/60 font-medium'}`}>
                                                                {item.day}
                                                            </span>
                                                        </div>
                                                    );
                        ...
                        <div className="grid grid-cols-3 gap-1 sm:gap-1.5 md:gap-2 flex-shrink-0">
                            <div className="bg-dash-lime rounded-md sm:rounded-lg md:rounded-xl p-1.5 sm:p-2 md:p-3 flex flex-col justify-between min-h-[50px] sm:min-h-[60px] md:min-h-[80px]">
                                <h3 className="text-[9px] sm:text-[10px] md:text-xs font-semibold text-dash-dark tracking-tight">Sessions</h3>
                                <p className="text-sm sm:text-base md:text-lg lg:text-xl font-semibold text-dash-dark tracking-tight leading-none">{MOCK_DASHBOARD_DATA.stats.sessions}</p>
                            </div>

                            <div className="bg-dash-lime rounded-md sm:rounded-lg md:rounded-xl p-1.5 sm:p-2 md:p-3 flex flex-col gap-0.5 sm:gap-1 min-h-[50px] sm:min-h-[60px] md:min-h-[80px]">
                                <h3 className="text-[9px] sm:text-[10px] md:text-xs font-semibold text-dash-dark tracking-tight">Average</h3>
                                <div className="bg-dash-dark/20 rounded py-0.5 sm:py-1 flex items-center justify-center mt-auto">
                                    <span className="text-[9px] sm:text-[10px] md:text-xs font-semibold text-dash-dark tracking-tight leading-none">{MOCK_DASHBOARD_DATA.stats.average}</span>
                                </div>
                            </div>

                            <div className="bg-dash-lime rounded-md sm:rounded-lg md:rounded-xl p-1.5 sm:p-2 md:p-3 flex flex-col gap-0.5 sm:gap-1 min-h-[50px] sm:min-h-[60px] md:min-h-[80px]">
                                <h3 className="text-[9px] sm:text-[10px] md:text-xs font-semibold text-dash-dark tracking-tight">Longest</h3>
                                <div className="bg-dash-dark/20 rounded py-0.5 sm:py-1 flex items-center justify-center mt-auto">
                                    <span className="text-[9px] sm:text-[10px] md:text-xs font-semibold text-dash-dark tracking-tight leading-none">{MOCK_DASHBOARD_DATA.stats.longest}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5 sm:gap-2 md:gap-3 h-full md:row-span-2 lg:row-span-1">
                        <div className="bg-dash-dark rounded-lg sm:rounded-xl md:rounded-2xl p-1.5 sm:p-2 md:p-2.5 flex flex-col justify-between flex-shrink-0">
                            <div className="flex justify-between items-start">
                                <div>
                                    <h2 className="text-xs sm:text-xs md:text-sm lg:text-base font-semibold text-dash-lime tracking-tight leading-tight">Daily Goal</h2>
                                    <p className="text-[10px] sm:text-[11px] md:text-sm text-dash-moss">{MOCK_DASHBOARD_DATA.dailyGoal.target}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-xs sm:text-sm lg:text-base font-bold text-dash-lime">{MOCK_DASHBOARD_DATA.dailyGoal.progress}%</p>
                                </div>
                            </div>
                            <div className="flex justify-center mt-1 sm:mt-2 lg:mt-4 relative h-12 sm:h-16 md:h-20 lg:h-24 overflow-hidden">
                                <svg className="w-16 h-16 sm:w-24 sm:h-24 md:w-32 md:h-32 lg:w-40 lg:h-40 -rotate-[180deg]" viewBox="0 0 100 100">
                                    ...
                                    <circle
                                        ...
                                        strokeLinecap="round"
                                        className="text-dash-lime transition-all duration-1000 ease-out"
                                    />
                                </svg>
                                <div className="absolute bottom-0 left-0 right-0 flex flex-col items-center justify-end pb-1">
                                    <p className="text-sm sm:text-lg md:text-xl lg:text-2xl font-bold text-dash-lime leading-none">{MOCK_DASHBOARD_DATA.dailyGoal.progress}%</p>
                                    <p className="text-[8px] sm:text-[9px] md:text-[10px] text-dash-moss font-bold uppercase tracking-widest">Progress</p>
                                </div>
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-1.5 sm:gap-2 flex-1">
                            <div className="bg-dash-dark rounded-md sm:rounded-lg md:rounded-xl p-1.5 sm:p-2 relative overflow-hidden min-h-[50px] sm:min-h-[60px]">
                                <h2 className="text-[10px] sm:text-xs md:text-sm font-semibold text-dash-lime leading-tight relative z-20">Distractions</h2>
                                <p className="text-[9px] sm:text-[10px] font-semibold text-dash-olive relative z-20">Blocked</p>
                                <div
                                    className="absolute bg-dash-lime rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 ease-out z-0"
                                    style={{
                                ...
                                >
                                    <span
                                        className="font-black text-dash-dark tracking-tighter leading-none select-none transition-all duration-300"
                                        style={{
                            ...
                            <div className="bg-dash-dark rounded-md sm:rounded-lg md:rounded-xl p-1.5 sm:p-2 flex flex-col gap-1 overflow-hidden">
                                <div className="flex flex-col gap-0.5">
                                    <h2 className="text-[12px] sm:text-sm md:text-base font-bold text-dash-lime leading-tight">Top Distractions</h2>
                                    {windowWidth >= 1024 && (
                                        <p className="text-[10px] sm:text-[11px] font-medium text-dash-moss">This Week</p>
                                    )}
                                </div>
                                ...
                                {windowWidth >= 1024 ? (
                                    ...
                                    <div
                                        className="flex w-full mt-0.5 bg-dash-dark/30 gap-1.5 items-center relative z-20"
                                    ...
                                    >
                                        {MOCK_DASHBOARD_DATA.distractions.topDistractions.map((item, i) => (
                                            ...
                                            >
                                                <div
                                                    className={`w-full h-full rounded-full transition-all duration-300 hover:brightness-125 cursor-help shadow-sm ${['bg-dash-lime', 'bg-dash-moss', 'bg-dash-olive', 'bg-dash-forest'][i % 4]
                                                        }`}
                                                />
                                            </div>
                                        ))}
                                        ...
                                        {MOCK_DASHBOARD_DATA.distractions.topDistractions.map((item, i) => (
                                            <div key={i} className="flex items-center gap-2">
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <div
                                                            className={`h-full rounded-full shadow-sm ${['bg-dash-lime', 'bg-dash-moss', 'bg-dash-olive', 'bg-dash-forest'][i % 4]}`}
                                                            style={{
                                                    ...
                                                    >
                                                        <div className="bg-dash-lime text-dash-dark text-[10px] font-bold px-2 py-1 rounded shadow-lg whitespace-nowrap select-none">
                                                ...
                                                <p className="text-[10px] sm:text-[10px] font-medium text-dash-moss tracking-tight whitespace-nowrap !bg-transparent !bg-none !border-none !shadow-none select-none m-0 p-0 leading-none">
                                                    {item.name}
                                                </p>
                                            </div>
                                        ))}
                                ...
                                >
                                    {hoveredDistIndex !== null && (() => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <span
                                                    className="font-black text-dash-lime uppercase tracking-[0.2em] leading-none"
                                                ...
                                                <span
                                                    className="font-bold text-dash-moss uppercase tracking-widest leading-none"
                                                ...
                                                <span
                                                    className="font-medium text-dash-olive uppercase tracking-[0.15em] leading-none"
                                        ...
                                        {MOCK_DASHBOARD_DATA.distractions.topDistractions.map((item, i) => {
                                            ...
                                            return (
                                                <div key={i} className="flex items-center group/item" style={{ gap: `${fontSize * 0.3}px` }}>
                                                    <div
                                                        className={`rounded-full flex-shrink-0 shadow-sm transition-transform group-hover/item:scale-110 ${['bg-dash-lime', 'bg-dash-moss', 'bg-dash-olive', 'bg-dash-forest'][i % 4]
                                                            }`}
                                                        style={{
                                                            width: `${dotSize}px`,
                                                            height: `${dotSize}px`
                                                    ...
                                                    />
                                                    <span
                                                        className="font-medium text-dash-moss tracking-tight truncate transition-colors group-hover/item:text-dash-lime leading-tight"
                        ...
                        <div className="bg-dash-dark rounded-lg sm:rounded-xl md:rounded-2xl p-2 sm:p-2.5 md:p-3 flex-shrink-0 min-h-[120px] overflow-hidden flex flex-col">
                            <h2 className="text-sm sm:text-base md:text-lg font-semibold text-dash-lime tracking-tight leading-tight mb-1.5 sm:mb-2 flex-shrink-0">Task List</h2>
                            <div className="space-y-1 sm:space-y-1.5 overflow-y-auto pr-1 sm:pr-1.5 custom-scrollbar flex-1 max-h-[150px] lg:max-h-none">
                                {MOCK_DASHBOARD_DATA.tasks.map((task, i) => (
                                    <div key={i} className="flex items-center gap-2 sm:gap-2.5 md:gap-3 bg-dash-forest rounded-lg sm:rounded-xl md:rounded-2xl p-2 sm:p-2.5 md:p-3 hover:bg-dash-forest/80 transition-colors cursor-pointer group">
                                        <div className={`w-[16px] h-[16px] sm:w-[20px] sm:h-[20px] md:w-[24px] md:h-[24px] rounded-md flex-shrink-0 flex items-center justify-center transition-all ${task.completed ? 'bg-dash-lime' : 'bg-dash-dark border border-dash-lime/20 group-hover:border-dash-lime/40'}`}>
                                            {task.completed && (
                                                <svg className="w-3 h-3 sm:w-4 sm:h-4 text-dash-dark" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                                                </svg>
                                            )}
                                        </div>
                                        <span className={`text-[10px] sm:text-xs md:text-sm transition-all ${task.completed ? 'text-dash-lime/50 line-through' : 'text-dash-lime'}`}>
                                            {task.title}
                                        </span>
                                    </div>
                                ))}
                        ...
                        <div className="block lg:hidden bg-dash-dark rounded-lg sm:rounded-xl md:rounded-2xl p-2 sm:p-3 md:p-4 flex flex-col gap-1.5 sm:gap-2 md:gap-3 flex-1 lg:flex-shrink-0 justify-center">
                            {isAuthenticated && track ? (
                                <div className="flex flex-col gap-2">
                                    <div className="text-center">
                                        <p className="text-dash-lime font-bold text-xs sm:text-sm truncate">{track.name}</p>
                                        <p className="text-dash-moss text-[10px] sm:text-xs truncate">{track.artist}</p>
                                    </div>

                                    <div className="flex justify-center items-center gap-3 sm:gap-4">
                                        <button
                                            onClick={skipPrevious}
                                            className="p-1 sm:p-1.5 hover:bg-dash-lime/10 active:scale-95 rounded-full transition-all flex items-center justify-center h-6 w-6 sm:h-7 sm:w-7 text-dash-lime"
                                            title="Previous"
                                        >
                                        ...
                                        <button
                                            ...
                                            className="w-8 h-8 sm:w-9 sm:h-9 bg-dash-lime active:scale-90 rounded-full flex items-center justify-center hover:scale-105 transition-all shadow-lg shrink-0"
                                            title={track.is_playing ? 'Pause' : 'Play'}
                                        >
                                            {track.is_playing ? (
                                                <Pause className="w-4 h-4 sm:w-4.5 sm:h-4.5 fill-dash-dark stroke-dash-dark" />
                                            ) : (
                                                <Play className="w-4 h-4 sm:w-4.5 sm:h-4.5 fill-dash-dark stroke-dash-dark ml-0.5" />
                                            )}
                                        </button>
                                ...
                                <div className="flex flex-col items-center justify-center text-center gap-2">
                                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-dash-lime/20 rounded-full flex items-center justify-center">
                                        <svg className="w-5 h-5 sm:w-6 sm:h-6 text-dash-lime" viewBox="0 0 24 24" fill="currentColor">
                                    ...
                                    <div>
                                        <h3 className="text-dash-lime font-bold text-xs sm:text-sm">Spotify</h3>
                                        <p className="text-dash-moss text-[10px] sm:text-xs">Connect to see what's playing</p>
                                    </div>
                                    <button
                                        onClick={login}
                                        className="bg-dash-lime text-dash-dark px-3 py-1 sm:px-4 sm:py-1.5 rounded-full font-bold text-[10px] sm:text-xs hover:bg-dash-lime/90 transition-colors"
                                    >
                                        Connect
                                    </button>
                                </div>
                    ...
                    <div className="flex flex-col gap-2 sm:gap-3 md:gap-4 min-h-0">
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 w-full flex-shrink-0">
                            <div className="flex-1 h-[28px] sm:h-[32px] md:h-[36px] px-2 sm:px-3 md:px-4 flex items-center justify-between bg-dash-lime rounded-lg sm:rounded-xl">
                                <span className="text-[8px] sm:text-[9px] md:text-[10px] font-bold text-dash-dark uppercase">Current Streak</span>
                                <span className="text-[10px] sm:text-xs md:text-sm font-bold text-dash-dark">{MOCK_DASHBOARD_DATA.streak}</span>
                            </div>
                            <div className="w-[28px] h-[28px] sm:w-[32px] sm:h-[32px] md:w-[36px] md:h-[36px] flex items-center justify-center bg-dash-lime rounded-full">
                                <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4" viewBox="0 0 28 28" fill="none">
                                    <path d="M15.0973 21.5754C17.6715 21.0616 21 19.2179 21 14.4502C21 10.1115 17.8102 7.22242 15.5165 5.89491C15.0076 5.60033 14.4118 5.98773 14.4118 6.57386V8.07306C14.4118 9.25527 13.9125 11.4132 12.5253 12.3107C11.8171 12.769 11.0522 12.0831 10.9661 11.2465L10.8954 10.5596C10.8133 9.76097 9.99634 9.27617 9.35526 9.76308C8.20356 10.6378 7 12.1695 7 14.4502C7 20.2807 11.3556 21.7384 13.5333 21.7384C13.66 21.7384 13.7931 21.7346 13.9317 21.7267C12.8564 21.6352 11.1176 20.9709 11.1176 18.8228C11.1176 17.1426 12.3489 16.0059 13.2844 15.4533C13.5359 15.3047 13.8304 15.4977 13.8304 15.7889V16.272C13.8304 16.6417 13.9741 17.2197 14.3159 17.6153C14.7028 18.0629 15.2706 17.594 15.3164 17.0052C15.3309 16.8195 15.5185 16.7011 15.6801 16.7951C16.2082 17.1024 16.8824 17.7588 16.8824 18.8228C16.8824 20.502 15.9526 21.2745 15.0973 21.5754Z" className="fill-dash-dark" />
                                </svg>
                            </div>
                        </div>

                        <div className="bg-dash-dark rounded-lg sm:rounded-xl md:rounded-2xl p-2 sm:p-3 md:p-4 flex flex-col flex-1 lg:flex-shrink-0 min-h-0">
                            <div className="flex justify-between items-center mb-2 sm:mb-3">
                                <h3 className="text-[10px] sm:text-xs md:text-sm font-bold text-dash-lime uppercase">
                                    {format(currentMonth, 'MMMM yyyy')}
                                </h3>
                                <div className="flex gap-1 sm:gap-1.5">
                                    <button
                                        onClick={handlePrevMonth}
                                        className="p-0.5 sm:p-1 hover:bg-dash-lime/10 rounded transition-colors"
                                    >
                                        <svg className="w-2 h-2 sm:w-2.5 sm:h-2.5" viewBox="0 0 10 10" fill="none">
                                            <path d="M5.80983 0.463481C6.05104 0.211 6.45422 0.211 6.69543 0.463481C6.92158 0.700182 6.92158 1.07288 6.69543 1.30958L3.63022 4.51793L6.69543 7.72628C6.92158 7.96298 6.92158 8.33567 6.69543 8.57238C6.45422 8.82486 6.05104 8.82486 5.80983 8.57238L1.93626 4.51793L5.80983 0.463481Z" className="fill-dash-olive" />
                                        </svg>
                                    </button>
                                    <button
                                    ...
                                    >
                                        <svg className="w-2 h-2 sm:w-2.5 sm:h-2.5" viewBox="0 0 10 10" fill="none">
                                            <path d="M3.22603 0.463481C2.98481 0.211 2.58164 0.211 2.34042 0.463481C2.11428 0.700182 2.11428 1.07288 2.34042 1.30958L5.40564 4.51793L2.34042 7.72628C2.11428 7.96298 2.11428 8.33567 2.34042 8.57238C2.58164 8.82486 2.98481 8.82486 3.22603 8.57238L7.09959 4.51793L3.22603 0.463481Z" className="fill-dash-lime" />
                                        </svg>
                                    </button>
                                </div>
                            </div>
                            <div className="grid grid-cols-7 mb-1 text-center shrink-0">
                                {['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su'].map(d => (
                                    <div key={d} className="text-[8px] sm:text-[9px] md:text-[10px] font-bold text-dash-olive">{d}</div>
                                ))}
                            </div>
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`text-[8px] sm:text-[9px] md:text-[10px] py-0.5 sm:py-1 font-semibold transition-all ${isTodayDate
                                                ? 'bg-dash-lime text-dash-dark rounded-md'
                                                : !isCurrentMonth
                                                    ? 'text-dash-lime/20'
                                                    : 'text-dash-lime'
                                                }`}
                            ...
                            {isAuthenticated ? (
                                isIdle && user ? (
                                    ...
                                    <>
                                        ...
                                        <div className="text-center flex flex-col gap-1 sm:gap-1.5 md:gap-2 relative z-20">
                                            <div className="mb-0.5 sm:mb-1">
                                                <p className="text-dash-lime font-bold text-xs sm:text-sm md:text-base truncate">{track.name}</p>
                                                <p className="text-dash-moss text-[10px] sm:text-xs truncate">{track.artist}</p>
                                            </div>
                                            ...
                                            <div className="w-full group">
                                                <input
                                                    ...
                                                    className="w-full h-0.5 sm:h-1 bg-dash-olive/20 rounded-full appearance-none cursor-pointer accent-dash-lime transition-all"
                                                />
                                                <div className="flex justify-between mt-0.5 sm:mt-1 px-0.5">
                                                    <span className="text-dash-moss text-[8px] sm:text-[9px] md:text-[10px] font-medium tabular-nums">
                                            ...
                                            <div className="flex justify-center items-center gap-3 sm:gap-4 md:gap-6 min-h-[32px] sm:min-h-[40px]">
                                                <button
                                                    onClick={skipPrevious}
                                                    className="p-1 sm:p-1.5 md:p-2 hover:bg-dash-lime/10 active:scale-95 rounded-full transition-all flex items-center justify-center h-6 w-6 sm:h-8 sm:w-8 text-dash-lime"
                                                    title="Previous"
                                                >
                                                ...
                                                <button
                                                    onClick={togglePlayback}
                                                    className="w-7 h-7 sm:w-9 sm:h-9 md:w-11 md:h-11 bg-dash-lime active:scale-90 rounded-full flex items-center justify-center hover:scale-105 transition-all shadow-lg shrink-0"
                                                    title={track.is_playing ? 'Pause' : 'Play'}
                                                >
                                                    {track.is_playing ? (
                                                        <Pause className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 fill-dash-dark stroke-dash-dark" />
                                                    ) : (
                                                        <Play className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 fill-dash-dark stroke-dash-dark ml-0.5" />
                                                    )}
                                                </button>
                                    ...
                                    <div className="flex-1 flex flex-col items-center justify-center text-center gap-2 sm:gap-3">
                                        <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 bg-dash-lime/20 rounded-full flex items-center justify-center">
                                            <svg className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-dash-lime" viewBox="0 0 24 24" fill="currentColor">
                                                <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.485 17.302c-.215.354-.675.466-1.03.25-2.857-1.745-6.453-2.14-10.687-1.173-.406.093-.815-.16-.908-.567-.093-.406.16-.815.567-.908 4.636-1.06 8.594-.61 11.808 1.353.354.215.466.675.25 1.03zm1.464-3.26c-.27.44-.847.58-1.287.31-3.27-2.01-8.254-2.59-12.12-1.415-.494.15-1.025-.13-1.175-.624-.15-.494.13-1.025.624-1.175 4.414-1.34 9.907-.695 13.65 1.616.44.27.58.847.31 1.287zm.126-3.41c-3.922-2.33-10.385-2.545-14.136-1.406-.6.182-1.24-.16-1.423-.762-.182-.6.16-1.24.762-1.423 4.314-1.31 11.448-1.055 15.952 1.62.54.32.716 1.025.397 1.566-.32.54-1.025.716-1.566.397z" />
                                            </svg>
                                        </div>
                                        <div>
                                            <h3 className="text-dash-lime font-bold text-xs sm:text-sm md:text-base">Spotify Connected</h3>
                                            <p className="text-dash-moss text-[10px] sm:text-xs">Waiting for playback...</p>
                                        </div>
                                        <button
                                            onClick={togglePlayback}
                                            className="bg-dash-lime text-dash-dark px-3 py-1 sm:px-4 sm:py-1.5 md:px-6 md:py-2 rounded-full font-bold text-[10px] sm:text-xs md:text-sm hover:bg-dash-lime/90 transition-colors"
                                        >
                                            Resume
                                        </button>
                                    </div>
        ...
        .custom-scrollbar::-webkit-scrollbar-track { background: rgba(101, 120, 46, 0.05); border-radius: 10px; }
        .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(196, 217, 70, 0.4); border-radius: 10px; }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: rgba(196, 217, 70, 0.6); }
      `}} />
        </div>
    );
}
